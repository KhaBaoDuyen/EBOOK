    ...
    const handleLibrary = async () => {
        if (!user) {
            setNotify({
                ...
                message: " Hiện tại bạn chưa đăng nhập vui lòng đăng nhập để có thể sử dụng tiện ích"
            });
            return;
        }
        ...
        if (!userId || !bookId) {
            console.error("Thiếu userId hoặc bookId:", { userId, bookId });
            return;
        }
        try {
            await createLibrary(userId, bookId);
            navigate(`/render/${book?.slug}`);
        } catch (error) {
            setNotify({
                open: true,
                type: "error",
                title: "Không thể khởi tạo thư viện",
